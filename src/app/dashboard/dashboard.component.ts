...
export class DashboardComponent implements OnInit {
  ...
  users: Array<User>; // users list.
  public messages: Array<Message> = [] // messages array/
  ...
  getAllUsers() {
    ...
    this.api.getUsers().pipe(
      ...
      this.users = data.filter((item) => {
        ...
        let find = false;
        if (this.api.currentUser.conversation) {
          find = this.api.currentUser.conversations.find(el => el.uid == item.uid);
        }
        if (!find && this.api.currentUser.uid !== item.uid) {
          return item;
        }
